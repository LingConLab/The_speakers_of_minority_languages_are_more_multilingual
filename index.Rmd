...
# Suplementary Material for 'The speakers of minority languages are more multilingual' {.tabset .tabset-fade .tabset-pills}
...
fit <- glmer(sum_langs~sex*census_1926_modified+(1|decade) + (1|residence.en), data = df_reg, family = "poisson")
...
data.frame(decade = rownames(ranef(fit)$decade),
           intercept = ranef(fit)$decade) %>% 
  write_csv("decade_random_effects.csv")
```
